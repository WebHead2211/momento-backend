...
const logoutUser = async (req, res, next) => {
  try {
    await User.findByIdAndUpdate(
      ...
      //this will send back the user after updating if required
      //{new: true}
    );
...
const editUser = async (req, res, next) => {
  try {
    ...
    const avatarLocalPath = req.files?.avatar?.[0]?.path;
    if (avatarLocalPath) {
      let oldAvatar = user.avatar.split("/");
      let oldAvatarId = oldAvatar[oldAvatar.length - 1].split(".")[0];
      const avatar = await uploadOnCloudinary(avatarLocalPath);
      if (!avatar) {
        throw new ApiError(400, "Could not upload avatar on cloudinary");
      }
      user.avatar = avatar.url;
      const deletedAvatar = await deleteFromCloudinary(oldAvatarId);
      // console.log("DELETED FILE: ", deletedAvatar);
    }
    await user.save();
